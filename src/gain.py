...
#use the appropriate data files and nonlin file
superbias_used = True
...
def getuid(fname):
    if '/' not in fname:
        fname=join("/nirps_raw/nirps/reads",fname)
    return fits.getheader(fname)["HIERARCH ESO DET RAMP ID"]
class gain():
    def __init__(self):
        ...
        self.uid1 = ''
        self.uid2 = ''
        #self.superbias = "superbias220517095751.fits"
        
    def var_err(self,f,var):
        def unc(f):
            return np.sqrt(16**2+f)
        return var*2*unc(f)/f
    def makeXY(self,fname1,fname2,remakefile=False):
        self.uid1 = getuid(fname1)
        self.uid2 = getuid(fname2)
        #1st we check if the prepared files exist, if yes we return immediatly
        if isfile(join(self.pathtmp,"%sx%s.gain.diff.fits"%(self.uid1,self.uid2))) and not remakefile:
            self.diff = fits.getdata(join(self.pathtmp,"%sx%s.gain.diff.fits"%(self.uid1,self.uid2)))
            self.S = fits.getdata(join(self.pathtmp,"%sx%s.gain.s.fits"%(self.uid1,self.uid2)))
            print("%sx%s.gain.s.fits loaded."%(self.uid1,self.uid2))
            print("%sx%s.gain.diff.fits loaded."%(self.uid1,self.uid2))
            return
        if isfile(join(self.pathtmp,"%sx%s.gain.diff.fits"%(self.uid2,self.uid1))) and not remakefile:
            self.diff = fits.getdata(join(self.pathtmp,"%sx%s.gain.diff.fits"%(self.uid2,self.uid1)))
            self.S = fits.getdata(join(self.pathtmp,"%sx%s.gain.s.fits"%(self.uid2,self.uid1)))
            print("%sx%s.gain.s.fits loaded."%(self.uid2,self.uid1))
            print("%sx%s.gain.diff.fits loaded."%(self.uid2,self.uid1))
            return
        #We will need to create the files
        print("Creating files.")
        if superbias_used:
            ext=".bias.fits"
        else:
            ext=".fits"
        cube1 = fits.getdata(join(self.path,self.uid1+ext))
        cube2 = fits.getdata(join(self.path,self.uid2+ext))
        ...
        hdu.writeto(join(self.pathtmp,"%sx%s.gain.diff.fits"%(self.uid1,self.uid2)),overwrite=True)
        hdu = fits.PrimaryHDU(data=self.S)
        hdu.writeto(join(self.pathtmp,"%sx%s.gain.s.fits"%(self.uid1,self.uid2)),overwrite=True)
...
if '__main__' in __name__:
    ...
    flats= np.asarray(["NIRPS_2022-05-05T16_09_12_198.fits",
    ...
    ll = flats.reshape(8,2)
    for i in range(ll.shape[0]):
        print("%s - %s"%(ll[i,0],ll[i,1]))
