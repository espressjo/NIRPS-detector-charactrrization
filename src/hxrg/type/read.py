...
from hxrg.type.amp import hxrg_amp
from hxrg.fits2ramp.refpixcorr import refpxcorr
from hxrg.stats.stats import stats
...
class hxread(refpxcorr,stats):
    ...
    def get_deviant_pixels(self,pixel_bin=32):
    ...
    def fft(self,max_freq=200):
        ...
        for i in range(32):
            ...
            if amp.stats('median')==0.0:
                check = True
        
        for i in range(32):
            amp = self.get_amp(i+1)
            if not check:
                #only divide by median if no value are 0
